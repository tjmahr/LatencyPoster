...
Reaction times slower than `r round(too_slow)` ms will be removed. There are `r num_too_slow` such trials, which is `r round((num_too_slow / num_non_NA)*100)`% of the data.
...
p <- ggplot(subject_means, aes(x = Latency, fill = Version)) + geom_dotplot(method="histodot", stackgroups=FALSE) + scale_y_continuous(name = "", breaks = NULL)
print(p)
```
