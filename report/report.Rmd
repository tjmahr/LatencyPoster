...
```{r, fig.cap="Figure 2. Histograms of latencies (ms) for condition 1 (top) and condition 2 (bottom) for CP trials (left) and NW trials (right).", fig.height=7, fig.width=9, message=FALSE, warning=FALSE, echo=FALSE, dpi=300}
...
```{r, fig.cap="Figure 2. Histograms of latencies (in ms) for condition 1 (top) and condition 2 (bottom) for CP trials.", fig.height=7, fig.width=9, message=FALSE, warning=FALSE, echo=FALSE, dpi=300}
# d1 <- subset(results2, Condition == "real")
# qplot(data = d1, x = Latency) + facet_grid(Ver ~ .) + 
#   theme_bw() + labs(x = "Latency (ms)", y = "Count")+
#   theme(text = element_text(size = 20))
```

```{r, fig.cap="Figure 3. Histograms of latencies (in ms) for condition 1 (top) and condition 2 (bottom) for NW trials.", fig.height=7, fig.width=9, message=FALSE, warning=FALSE, echo=FALSE, dpi=300}
# d2 <- subset(results2, Condition == "nonsense")
# qplot(data = d2, x = Latency) + facet_grid(Ver ~ .) + 
#   theme_bw() + labs(x = "Latency (ms)", y = "Count") + 
#   theme(text = element_text(size = 20))
```
...
```{r, fig.cap="Figure 2. Relationship between EVT-2 and mean latencies for each subject by condition and trial", fig.height=6, fig.width=9, message=FALSE, warning=FALSE, echo=FALSE, dpi=300}
