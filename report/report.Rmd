...
# Prep table
row.names(scores) <- scores$Version
scores$Version <- NULL
names(scores) <- c("Age (months)", "EVT-2", "PPVT-4")
PrettyPrint(scores, header = FALSE)
```
...
```{r, fig.cap="Figure 2. Histograms of latencies (in ms) for condition 1 (top) and condition 2 (bottom) for CP trials.", fig.height=8, fig.width=9, message=FALSE, warning=FALSE, echo=FALSE}
...
```{r, fig.cap="Figure 3. Histograms of latencies (in ms) for condition 1 (top) and condition 2 (bottom) for NW trials.", fig.height=8, fig.width=9, message=FALSE, warning=FALSE, echo=FALSE}
